...
ip = "localhost"
porta = 8081
enderecosBrokers = [("localhost", 8080), ("localhost", 8086)]  # enderecos dos brokers
...
topicos = [[1, "Topico 1"], [2, "Topico 2"]]  # topicos
atualizacao = {1: [], 2: []}
acquires = {1: threading.Lock(), 2: threading.Lock()}
...
def notify(topico, mensagem):
    for top, sub in subEscritoresOnline:
        if int(top) == int(topico):
            ...
            sub.recv(buffer)  # receber confirmacao
...
def lidarComPublicador(publicador):
    ...
    acquires[topicoASerEscrito].release()
