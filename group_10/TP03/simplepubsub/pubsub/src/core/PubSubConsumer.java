...
public class PubSubConsumer<S extends Socket> extends GenericConsumer<S> {
    ...
    protected void doSomething(S str) {
        try {
            ...
            if (msg.getType().startsWith("syncAddr")) {
                response = commands.get(msg.getType()).execute(msg, log, subscribers, isPrimary, secondaryServer,
                        secondaryPort);
            } else if (msg.getType().startsWith("changeBroker")) {
                this.isPrimary = true;
                this.secondaryServer = msg.getContent().split(":")[0];
                this.secondaryPort = Integer.parseInt(msg.getContent().split(":")[1]);
