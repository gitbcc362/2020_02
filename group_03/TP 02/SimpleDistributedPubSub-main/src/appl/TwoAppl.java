...
public class TwoAppl {
	...
	public TwoAppl(boolean flag){
		...
		String clientHostGCP = "10.182.0.5";
		int DEFAULT_PORT = 8080;
		
		
		PubSubClient pedro = new PubSubClient(clientHostGCP, 8082);
		...
		Thread accessThree = new SynchronizationProtocol(pedro, "pedro_acquire_X", brokerHostIntern, DEFAULT_PORT );
		...
		List<Message> toRemove = removeWrongMessages(logpedro);
		logpedro.removeAll(toRemove);
		toRemove.clear();
		printLogMessages(logpedro, "Log Pedro Itens");
		
		
		Iterator<Message> it = logpedro.iterator();
