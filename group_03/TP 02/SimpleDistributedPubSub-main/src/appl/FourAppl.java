...
public class FourAppl {
	public List<Message> lastAcquiresLog = new ArrayList<Message>();
	...
	public FourAppl(boolean flag){
		...
		//PubSubClient lucas = new PubSubClient(clientHost, 8081);
		//PubSubClient pedro = new PubSubClient(clientHost, 8082);
		//PubSubClient maria= new PubSubClient(clientHost, 8083);
		PubSubClient clara = new PubSubClient(clientHost, 8084);
		
		//lucas.subscribe(clientHost, DEFAULT_PORT);
		//pedro.subscribe(clientHost, DEFAULT_PORT);
		//maria.subscribe(clientHost, DEFAULT_PORT);
		clara.subscribe(clientHost, DEFAULT_PORT);
		
		
		//Thread accessOne = new SynchronizationProtocol(lucas, "lucas_acquire_X", brokerHost, DEFAULT_PORT);				
		//Thread accessTwo = new SynchronizationProtocol(pedro, "pedro_acquire_X", brokerHost, DEFAULT_PORT );
		//Thread accessThree = new SynchronizationProtocol(maria, "maria_acquire_X", brokerHost, DEFAULT_PORT);
		...
		Thread accessFive = new SynchronizationProtocol(pedro, "pedro_acquire_X", brokerHost, DEFAULT_PORT );
		Thread accessSix = new SynchronizationProtocol(maria, "maria_acquire_X", brokerHost, DEFAULT_PORT);
		Thread accessSeven = new SynchronizationProtocol(clara, "clara_acquire_X", brokerHost, DEFAULT_PORT);
		Thread accessEight = new SynchronizationProtocol(lucas, "lucas_acquire_X", brokerHost, DEFAULT_PORT);
		...
		Thread accessNine = new SynchronizationProtocol(maria, "maria_acquire_X", brokerHost, DEFAULT_PORT);
		Thread accessTen = new SynchronizationProtocol(clara, "clara_acquire_X", brokerHost, DEFAULT_PORT);
		Thread accessEleven = new SynchronizationProtocol(lucas, "lucas_acquire_X", brokerHost, DEFAULT_PORT);
		Thread accessTwelve = new SynchronizationProtocol(pedro, "pedro_acquire_X", brokerHost, DEFAULT_PORT );
		...
		*/
		...
		//Removendo mensagens de localhost
		List<Message> toRemove = removeWrongMessages(logClara);
		logClara.removeAll(toRemove);
		toRemove.clear();
		printLogMessages(logClara, "Log Clara Itens");
	
		
		Iterator<Message> it = logClara.iterator();
		...
		//lucas.unsubscribe(clientHost, DEFAULT_PORT );
		//pedro.unsubscribe(clientHost, DEFAULT_PORT);
		//maria.unsubscribe(clientHost, DEFAULT_PORT);
