...
public class Broker {
    ...
    public Broker() {
        ...
        if (respBol){
            s = new Server(8080, true, secondaryIp, 8081);
            brokerThread = new ThreadWrapper(s);
            brokerThread.start();
        } else {
            s = new Server(8081, false, primaryIp, 8080);
            brokerThread = new ThreadWrapper(s);
            brokerThread.start();
            ...
            msgBroker.setContent(secondaryIp+ ":" + 8081);

            Message status = null;

            try {
                Client subscriber = new Client(primaryIp, 8080);
