...
public class PubSubConsumer<S extends Socket> extends GenericConsumer<S>{
	...
	protected void doSomething(S str) {
		try{
			...
			System.out.println(msg.getContent() + " " + msg.getType() + " " + msg.getLogId());
			...
			response.setContent("Message notified: " + msg.getContent());

			response.setType("notify_ack");

			if (!msg.getType().equals("sub"))
				response = commands.get(msg.getType()).execute(msg, log, subscribers);
